  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="relative flex items-center justify-center bg-white p-2">
        ...
        <img src={images[index]} alt={`${alt} ${index + 1}`} className="max-h-[56vh] w-full object-contain" />
      ...
      {images.length > 1 && (
        <div className="flex gap-2 overflow-x-auto">
          {images.map((src, i) => (
            <button key={src + i} onClick={() => setIndex(i)} className={`h-16 w-16 flex-shrink-0 overflow-hidden rounded-md border ${i === index ? 'ring-2 ring-rose-400' : 'border-slate-200'}`}>
